 ...
 * @return bool|null
 */
if ( ! function_exists( 'wppb_admin_notice' ) ){
...
add_action( 'admin_init', 'wppb_notice_ignore' );
if ( ! function_exists( 'wppb_notice_ignore' ) ){
function wppb_notice_ignore(){
  global $current_user;
  $theme_data  = wp_get_theme();
  $user_id   = $current_user->ID;
...
if ( ! function_exists( 'wppb_notice_scripts' ) ){
function wppb_notice_scripts(){  
            wp_enqueue_style('hunk-companion-notices', WPPB_URL.'notify/assets/css/notice.css');
        }
      }
add_action('admin_enqueue_scripts', 'wppb_notice_scripts');